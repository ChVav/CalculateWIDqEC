...
# under development for Shiny App, Last update 11 August 2022
...
# Keep option in for fixed intercept and slope, or using external calibration curve (e.g. on separate plate)
# but will have to reimplement so that in case of external calibration curve, also gblock is taken from separate plate
# 12 August 2022
...
calculate_pmr <- function(data,#experimentname,
  ...
  # note that this section works also using variable data (instead of curve), 
  # but as written now option user has the option to upload a calibration curve from a different plate
  ...
  if(all(targets_qCIN %in% targets)){
    results <- results %>%
      mutate(WIDqCIN = (RALYL + DPP6 + GSX1)) # calculate sumPMR for WID-qCIN
  }
  ...
  list_out[[2]] <- results #PMR + optional WIDqEC + WIDqEC outcome +WIDqCIN
